...
test.describe(('Logged in user successfully adds campaigns'), () => {
    ...
    test('User successfully Registered', async ({ page, constants }) => {
        ...
        await expect(successMessage).toContainText('Welcome! You have signed up successfully.');

        await campaignsPage.verifyAddingCampaign(name);
        await expect(successMessage).toContainText('Campaign was successfully created.');
